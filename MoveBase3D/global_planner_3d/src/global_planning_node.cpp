...
void rcvPointCloudCallBack(const sensor_msgs::PointCloud2 &pointcloud_map)
{
  ROS_INFO("map:rcvPointCloudCallBack");
  pcl::PointCloud<pcl::PointXYZ> cloud;
  pcl::fromROSMsg(pointcloud_map, cloud);
  worldCloud = cloud;
  // 这里调用的是class world下面的initGridMap
  // 这里的world已经实例化过了
  // 实例化的时候将下限设置为正无穷
  // 上限设置为负无穷
                         ...
                         double resolution)
{
  ...
  for (const auto &_pt : cloud.points)
  {
    ...
    for (const auto &world_pt : worldCloud.points)
    {
      ...
      {
        // 如果投影点在worldCloud内部 那么添加进projected_cloud
...
void callPlanner()
{
  ...
  // has_goal false  init_time_cost<1000
  ...
  {
    // NOTE:如果发布了目标点 那么调用findSolution
    findSolution();
    init_time_cost = 0.0;
  }
...
int main(int argc, char **argv)
{
  ...
  // fast lio计算得到的pose 在这里会将has_goal设置为true 然后会计算距离
  goal_sub = nh.subscribe("/cur_goal", 1, rcvGoalCallBack);
  // 障碍物 这里的障碍物由global_planning_obs发布出去
  // 由rcvLidarCallBack发布 分析接收到的实时雷达数据判断是否是障碍物 
  ...
  // Initialization
  // world使用resolution 初始化  resolution为分辨率
  world = new World(resolution);
  // 这里的h_surf_car是规划中的表面与车底盘的距离
  // world 在rcvPointCloudCallBack中初始化了
  // 这里实例化一个PFRRTStar
  ...
  /**
   * fit_plane_arg 里面包含很多参数
   * w_total_
   * w_flatness_ 平坦度的权重系数 表示对路径平坦度的偏好。值越大，越倾向于选择较平坦的路径
   * w_slope_
   * 
   */
  ...
  while (ros::ok())
  {
    ...
    while (true && ros::ok())
    {
      ...
      // if (listener.waitForTransform("/odom", "/base_footprint", ros::Time(0), ros::Duration(1.0)))
      {
        try
        {
          listener.lookupTransform("/map", "/base_link", ros::Time(0), transform); // 查询变换
          // listener.lookupTransform("/odom", "/base_footprint", ros::Time(0), transform); // 查询变换
    ...
    // start_pt在这里被赋值 被transfor赋值 值不为空 但是
    // ROS_INFO("start_pt:x:%f,y:%f,z:%f",transform.getOrigin().x(),transform.getOrigin().y(),transform.getOrigin().z());
    start_pt << transform.getOrigin().x(), transform.getOrigin().y(), transform.getOrigin().z();
    // 这里的start_pt是有值的

    // NOTE:has_goal走这条 has goal是在回调函数rcvGoalCallBack中设置为true的
    if (has_goal)
    {
