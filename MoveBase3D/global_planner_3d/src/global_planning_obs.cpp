...
{
  // 
  ROS_INFO("rcvLidarCallBack");
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
  pcl::fromROSMsg(lidar_points, *cloud);

  if (cloud->points.empty())
  {
    ROS_WARN("Raceived point cloud is empty");
  }
  
// map:rcvPointCloudCallBack
  // 设置 VoxelGrid 滤波器
  ...
  for (const auto &pt : cloud_filt->points)
  {
    ...
    // 目标坐标系
    // origin_point 要进行坐标变换的初始的点
    // trans_point 变换后的点
    // 这里是将雷达坐标系下面的点转换到map坐标系下面
    ...
    {
      // 判断是否包含在一个体素内
      // 检查_pt.x是否在 pt.x - resolution / 2和pt.x + resolution / 2之间
      // 将这一个点的八邻域全都加进去
  ...
  // if (!obs_array.layout.dim.empty()) {
  //     std::vector<size_t> shape;
  //     for (const auto& dim : obs_array.layout.dim) {
  //         shape.push_back(static_cast<size_t>(dim.size));
  //     }
      
  //     // 打印推断出的 shape
  //     ROS_INFO("Inferred shape:");
  //     for (size_t i = 0; i < shape.size(); ++i) {
  //         ROS_INFO("Shape[%zu]: %zu", i, shape[i]);
  //     }
  // } else {
  //     ROS_WARN("No dimension information available in the layout.");
  // }
...
int main(int argc, char **argv)
{
  ...
  // 这里的map话题被重映射了  重映射成了/rtabmap/cloud_map
