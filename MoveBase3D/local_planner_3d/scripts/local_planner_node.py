...
class Local_Planner():
    ...
    def obs_cb(self, data:Float32MultiArray):
        self.ob = []
        # dim=data.layout.dim
        # print(f"data的dim:{dim}")
    ...
    def evaluate_goal_cost(self,trajectory, goal_x, goal_y):
        ...
        """
        # trajectory是模拟的轨迹
        # 这里的trajectory是模拟的轨迹格式还是[[x,y,theta]]
        # 这里拿到的是最后一次的模拟结果
    ...
    def localPlan(self,cur_position, goal_position, obstacles, cur_v=0.0, cur_omega=0.0):
        ...
        for v in np.arange(dynamic_window["v_min"], dynamic_window["v_max"], 0.1):
            for omega in np.arange(dynamic_window["omega_min"], dynamic_window["omega_max"], 0.1):
                ...
                # HACK:不理解 先得到模拟的轨迹 模拟的轨迹是一段时间的轨迹
                # 这里会得到模拟的轨迹
                ...
                # 这里评价轨迹trajectory 轨迹是二维的 维度是(predict_time/dt,3)
                # obstacles 是从localPlan中传入的
                # 传入的是self.ob 
                # self.ob是在obs_cb函数中定义的
        ...
        print(f"best control{np.array(best_control)}")
    ...
    def pub_cmd(self):
        # 将best_control传给cmd函数
        # 只要调用了pub_cmd就会有速度消息
        self.cmd(self.best_control)

        try:
            # 如果没有这个坐标会返回异常
            # print(f"self.map_frame_id:{self.map_frame_id},self.base_frame_id:{self.base_frame_id}")
